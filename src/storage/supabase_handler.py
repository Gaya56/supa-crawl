...
class SupabaseHandler:
        ...
        if env_config.has_supabase_config:
            try:
                ...
                self.client = create_client(
                    env_config.supabase_url, env_config.supabase_key
                )
                print(
                    f"✓ Supabase client initialized: {env_config.supabase_url[:50]}..."
                )
    ...
    def _extract_first_paragraph(self, markdown_content: str) -> str:
        ...
        for paragraph in paragraphs:
            ...
            if len(clean_paragraph) > 50:  # Meaningful content
                ...
                return (
                    clean_paragraph[:500] + "..."
                    if len(clean_paragraph) > 500
                    else clean_paragraph
                )
        ...
        for result in results:
            try:
                ...
                raw_markdown = (
                    ...
                    if isinstance(result, dict)
                    else getattr(result, "raw_markdown", None)
                )
                analysis = (
                    result.get("analysis")
                    if isinstance(result, dict)
                    else getattr(result, "analysis", None)
                )
                ...
                if analysis:
                    title = (
                        analysis.get("title", "No title")
                        if isinstance(analysis, dict)
                        else getattr(analysis, "title", "No title")
                    )
                    summary = (
                        analysis.get("summary", "No summary")
                        if isinstance(analysis, dict)
                        else getattr(analysis, "summary", "No summary")
                    )
    ...
    def store_page_summary(
        ...
        try:
            ...
            data = {"url": url, "title": title, "summary": summary}
    ...
    def store_prediction_market_data(
        self, source_url: str, prediction_data: "PredictionMarketBet"
    ) -> Optional[Dict[str, Any]]:
        """
        Store prediction market data using the Testing table schema.
        Source: https://supabase.com/docs/reference/python/upsert

        Args:
            source_url: URL where the prediction market data was scraped from
            prediction_data: PredictionMarketBet object with extracted market data
        ...
        try:
            # Prepare data matching enhanced Testing table schema with all 12 columns
            # Columns: id, timestamp, source_url, website_name, bet_title, odds, summary, 
            #          market_category, betting_options, probability_percentage, volume_info, closing_date
            data = {
                "source_url": source_url,
                "website_name": prediction_data.website_name,
                "bet_title": prediction_data.bet_title,
                "odds": prediction_data.odds,
                "summary": prediction_data.summary,
                # Enhanced descriptive fields from migration
                "market_category": prediction_data.market_category,
                "betting_options": prediction_data.betting_options,
                "probability_percentage": prediction_data.probability_percentage,
                "volume_info": prediction_data.volume_info,
                "closing_date": prediction_data.closing_date,
                # Note: id and timestamp are auto-generated by Supabase
            }

            # Official Supabase insert pattern for prediction market data
            # Source: https://supabase.com/docs/reference/python/insert
            response = self.client.table("Testing").insert(data).execute()

            if response.data:
                print(
                    f"✓ Stored prediction market: {prediction_data.bet_title} from {prediction_data.website_name}"
                )
                return response.data
            else:
                print(f"⚠ No data returned for prediction market: {source_url}")
                return None

        except Exception as e:
            print(
                f"✗ Failed to store prediction market data for {source_url}: {str(e)}"
            )
            return None
